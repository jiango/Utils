...
BOOL AES::Decrypt(BYTE *lpInBuf,int nlen, BYTE *lpOutBuf)
{
	int j = 0;
	if(nlen < 0x10)
	{
	...
	while(j < nlen)
	{
		...
		InvCipher(lpInBuf + j,lpOutBuf + j);
// 		for(int i = 0;i < 16;i++)
// 		{
// 			printf("%.2X",outdata[i]);
// 		}
...
BOOL AES::Encrypt(BYTE *lpInBuf,int nlen, BYTE *lpOutBuf)
{
	int j = 0;
	if(nlen < 0x10)
	{
